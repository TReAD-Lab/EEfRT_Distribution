...
hVal = 1; %1 for special condition of full model where parameters are taken at face value, 
%%value of 0 for "reward only" model that does not use probability information (probabilities drop to 1) 
...
clear Fit
Fit.Nparms = 2;
Fit.LB = [0 0];   %parameter bounds - minimum 
Fit.UB = [10 100];    %parameter bounds - maximum 
SampleTrials = 0; 
fprintf('Fitting subject %d out of %d...\n',s,Nsubjects)
    ...
    for iter = 1:nIt   % run 50 times from random initial conditions, to get best fit
        ...
        [res,lik] = ... 
            fmincon(@(x) run_SVmodel_2parameter(x(1),x(2)),...
...
xlsOutGLCSO = [Fit.Result.BestFit(s,:) SampleTrials thisBIC];
 ...
 cd(outputDir); 
 thisH = num2str(hVal);  
 OutfileName = ['hVal_' thisH]; 
 eval(['save ' OutfileName '_SVOutput.txt allOutputMatrix -ascii']);
